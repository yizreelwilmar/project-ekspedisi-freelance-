    ...
    <div class="card-header p-0 w-75 position-fixed mt-n4 mx-2 z-index-2">
        <div class="shadow-dark border-radius-lg d-flex px-5 pt-4 pb-3">
            <div class="col-8 d-flex align-items-center">
                ...
                <h6 class="mb-0">User Data</h6>
            </div>
            <div class="col-4 text-end">
                <a href="<?= site_url('admin/user/adduser'); ?>" class="btn btn-info btn-sm mb-0" title="Add User">Add User</a>
            </div>
        </div>
    </div>
</div>
                            ...
                            <?php if (!empty($user)) : $i = 1;
                                foreach (array_reverse($user) as $value) : ?>
                                    <tr>
                                        ...
                                        <td class="pl-4">
                                            <span class="text-sm font-weight-bold"><?= $value->username; ?></span>
                                        </td>
                                        <td class="pl-4">
                                            <span class="text-sm font-weight-bold">
                                                <?= ($value->role == 'driver') ? 'User' : ucfirst($value->role); ?>
                                            </span>
                                        </td>

                                        <td class="pl-4">
                                            <a href="<?= site_url('admin/user/' . $value->user_id . '/updateuser'); ?>" rel="tooltip" title="delete" class="text-sm font-weight-bold text-info">
                                                <i class="material-icons text-sm">edit</i> Edit </a>
                                            <a href="<?= site_url('admin/deleteuser/' . $value->user_id . ''); ?>" rel="tooltip" title="delete" class="text-sm font-weight-bold text-danger">
