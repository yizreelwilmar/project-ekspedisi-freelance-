...
class Admin extends CI_Controller
{
    ...
    public function get_v_package_by_id()
    {
        $id_package = $this->input->post('id_package');
        $data['v_package'] = $this->crudModel->get_v_package_by_id($id_package);
    ...
    public function get_cities_by_province()
    {
        ...
        if ($id_provinsi) {
            ...
            if ($query) {
                ...
                echo json_encode(['error' => 'Failed to retrieve cities']);
            }
        } else {
            echo json_encode([]);
        }
    ...
    {
        $metodePembayaran = $this->input->post('payment_method'); // 'tunai', 'transfer', or 'hutang'
        $jangkaWaktu = $this->input->post('jangka_waktu'); // Only applicable if 'hutang' is selected
        $jumlahHutang = $this->input->post('jumlah_hutang'); // Only applicable if 'manual' is selected
        ...
        if ($metodePembayaran == 'hutang') {
            $add['jangka_waktu'] = $jangkaWaktu;

            if ($jangkaWaktu == 'manual') {
                $add['jumlah_hutang'] = trim($this->input->post('jumlah_hutang'));
            }
        }
    ...
    public function capacity()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            WHERE capacity.id_capacity = ' . $id . '  AND capacity.id_driver = driver.id_driver')->row();
            ...
            $data = [
                'capacity' => $this->db->query('SELECT * FROM capacity  JOIN driver
                              WHERE  capacity.id_driver = driver.id_driver ORDER BY cstatus DESC')->result(),
    ...
    public function addCapacitys()
    {
        ...
        $bmax =  $this->db->query('SELECT  b_capacity + 1000 as b_capacity FROM capacity  WHERE id_capacity = ' . $id_capacity . ' ')->row();
        ...
        $kota = isset($kota) ? $kota : 0; // Default to 0 if $kota is not set
        $date = isset($date) ? $date : ''; // Default to empty string if $date is not set

        // Ensure that $kota and $date are set and valid
        if (empty($kota) || empty($date)) {
            // Handle the error or provide a default behavior
            show_error('Invalid kota or date provided.');
        } else {
            $sql = 'SELECT id_package, id_kota, date, t_kg, t_kgv 
            FROM (
                SELECT id_package, 
                       id_kota, 
                       date, 
                ...
                WHERE id_kota = ? 
                  AND date = ? 
                  AND (pstatus = 1 OR pstatus = 3)
            ) cum_sum ';

            $cpciy = $this->db->query($sql, [
                $kota,
                $date
                //$vmax->s_capacity,
                //$bmax->b_capacity
        ...
        // $getb = $this->db->query('
        // SELECT v_capacity.id_capacity, v_capacity.id_package, SUM(t_kg) as t_kg
        // FROM v_capacity
        // JOIN capacity ON v_capacity.id_capacity = capacity.id_capacity
        // JOIN package ON v_capacity.id_package = package.id_package
        // WHERE v_capacity.id_capacity = ' . $id_capacity . '
        // GROUP BY v_capacity.id_capacity, v_capacity.id_package')->row();


        $getv = $this->db->query('
        SELECT v_capacity.id_capacity, v_capacity.id_package, SUM(t_kgv) as t_kgv
        FROM v_capacity
        JOIN capacity ON v_capacity.id_capacity = capacity.id_capacity
        JOIN package ON v_capacity.id_package = package.id_package
        WHERE v_capacity.id_capacity = ' . $id_capacity . '
        ...
        $getb = $this->db->query('SELECT 
                                        v_capacity.id_capacity, 
                                        v_capacity.id_package, 
                                        SUM(t_kg) AS t_kg,
                                        -- Calculate the total sum of t_kg for the specific id_capacity across all packages
                                        SUM(SUM(t_kg)) OVER (PARTITION BY v_capacity.id_capacity) AS total_tkg
                                    FROM 
                                        v_capacity
                                    JOIN 
                                        capacity ON v_capacity.id_capacity = capacity.id_capacity
                                    JOIN 
                                        package ON v_capacity.id_package = package.id_package
                                    WHERE 
                                        v_capacity.id_capacity = ' . $id_capacity . '
                                    GROUP BY 
                                        v_capacity.id_capacity, v_capacity.id_package')->row();


        $update = [
            'b_capacity' => $getb->total_tkg,
    ...
    // get user data -- Yizreel 
    public function user()
    {
        if ($this->uri->segment(3) === 'adduser') {
        ...
        } else if ($this->uri->segment(4) === 'updateuser') {
    ...
    // fungsi add user -- Yizreel
    public function adduser()
    {
        // Retrieve and trim the username input
        $username = trim($this->input->post('user'));

        // Debug: Check if username is properly retrieved
        if (empty($username)) {
            die('Username is empty or not set.');
        }

        $firstThreeChars = strtoupper(substr($username, 0, 3));

        $randomNumber = rand(10000, 99999); // Adjust the range as needed

        $userId = $firstThreeChars . $randomNumber;
        ...
        $add = [
            ...
            'email' => trim($this->input->post('email')),
            'cabid' => trim($this->input->post('cabid')),
        ];
    ...
    public function shipping()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $tampil = $this->db->query('SELECT * FROM capacity JOIN driver
            WHERE capacity.id_capacity = ' . $id . ' AND capacity.id_driver = driver.id_driver')->row();
            ...
            $data = [
                ...
                'capacity' => $this->db->query('SELECT * FROM capacity JOIN driver
                            WHERE capacity.id_driver = driver.id_driver AND cstatus = 2')->result(),
    ...
    public function receiveShipping()
    {
        ...
        $update2 = [
            'pstatus' => 4,
            'received_date' => date('Y-m-d H:i:s'), // Sets the current date and time
        ];
    ...
    public function report()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                'capacity' => $this->db->query('SELECT
                                                    c.*,
                                                    d.*,  
                                                    MAX(vc.id_package) AS id_package,
                                                    MAX(p.customer_name) AS package_name  
                                                FROM 
                                                    capacity c
                                                JOIN 
                                                    driver d ON c.id_driver = d.id_driver
                                                LEFT JOIN 
                                                    v_capacity vc ON c.id_capacity = vc.id_capacity
                                                LEFT JOIN 
                                                    package p ON vc.id_package = p.id_package
                                                WHERE 
                                                    c.cstatus = 3
                                                GROUP BY 
                                                    c.id_capacity
                                                ORDER BY c.id_capacity ASC')->result(),
    ...
    public function printSuratJalan()
    {
        ...
        $data = [
            ...
            'product' => $this->db->query('SELECT * FROM product')->result(),
            'v_package' => $this->db->query('SELECT * FROM v_package JOIN package JOIN product WHERE v_package.id_package = ' . $id . ' AND v_package.id_package = package.id_package AND v_package.id_product = product.id_product')->result(),
            'v_capacity' => $this->db->query('SELECT * 
                                                    FROM v_capacity 
                                                    JOIN package ON v_capacity.id_package = package.id_package 
                                                    JOIN kota ON package.id_kota = kota.id_kota 
                                                    JOIN capacity ON v_capacity.id_capacity = capacity.id_capacity 
                                                    JOIN driver ON capacity.id_driver = driver.id_driver 
                                                    WHERE package.no_package = ' . $no_pack . ' 
                                                    ORDER BY kota.priority DESC;')->result(),
        ];

        $this->load->view('ReportPackage', $data);
    }
    ...
    public function printTandaTerima()
    {
        ...
        $this->load->view('TandaTerima', $data);
    }
